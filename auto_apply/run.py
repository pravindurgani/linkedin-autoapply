...
@cli.command()
@click.option("--retry-failed", is_flag=True, help="Clear failed application records before running (re-queues them for retry)")
...
def run(retry_failed, skip_cv_review, dry_run, visible, max_applies):
    """Run the full pipeline: scrape, score, and apply."""
    if max_applies <= 0:
        raise click.UsageError("--max-applies must be a positive integer")
    if retry_failed:
        from auto_apply.store import reset_failed_applications
        reset_failed_applications()
