...
from auto_apply.config import DB_PATH, ensure_config_dir
    ...
    try:
        cur = conn.execute(
            """INSERT INTO jobs (title, company, location, salary_min, salary_max,
               ...
               WHERE excluded.title IS NOT title
                  OR excluded.company IS NOT company
                  OR excluded.location IS NOT location
                  OR excluded.description IS NOT description
                  OR excluded.salary_min IS NOT salary_min
                  OR excluded.salary_max IS NOT salary_max
                  OR excluded.salary_text IS NOT salary_text
                  OR excluded.url IS NOT url
                  OR excluded.easy_apply IS NOT easy_apply
            """,
